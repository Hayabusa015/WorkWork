...
GitHub stores source code and, on each release, the built installer. It does not sync documents, settings or API balance between computers. Signing is still future work; updates are manual — the in-app Check for updates button finds the newest release and installs it, and does nothing until at least one release exists.
