...
|---|---|---|
| 2026-09-14 | **Archivo cannot set charge superscripts.** `audit_fonts.py` on the first U14 S14.1 build named them: U+207A `⁺` (×24) and U+207B `⁻` (×13) pull DejaVu, so every ionic formula — H⁺, OH⁻, NH₄⁺, SO₄²⁻ — breaks the one-face rule. Archivo *does* have superscript digits (¹²³) and subscripts (₂₃₄), which is why the mole worksheets were fine. The decks now write `H+` / `OH-` on the baseline, which is legible but not how chemistry is set. The real fix is a superscript run (baseline shift) in `build.js` rather than a Unicode codepoint — a template change, so a proposal. U+21CC `⇌` is also missing; `↔` is supported and is used instead. | User — affects all of U14 and any ionic chemistry |
