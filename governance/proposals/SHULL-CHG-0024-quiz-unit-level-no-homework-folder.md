...
| **Status** | **APPROVED**, edits made, **not yet committed**. This Secretary session has no shell/git tool (tool set: Read, Glob, Grep, Write, Edit only) and cannot run `git add` / `git commit` / `git push` itself — the same limitation the prior session recorded for SHULL-CHG-0023. The orchestrating session must commit on this session's behalf, exactly as it did for SHULL-CHG-0023. |
| **Implemented By** | *(pending — to be filled in with the commit SHA once the orchestrating session commits these edits, citing `SHULL-CHG-0024`)* |
| **Verified** | Not yet. Once committed: `standards/DRIVE_ARCHITECTURE.md` §1 tree, grammar rule, "What goes where" table, and routing rule 2 are updated to `Quiz` (unit-level) / `Labs-Case Studies-Projects` (section-level, only one) / no `Homework` folder at any level; `config/drive.json` → `folderGrammar.unitContentFolders` (3), `folderGrammar.sectionContentFolders` (1), `contentFolderRule` and new `homeworkFiling` key are updated, and `$comment` and the Chemistry Unit 00 `knownDefects` entry are updated to match, all citing this ID; `standards/NAMING.md` was re-checked and requires no edit (unchanged reasoning from SHULL-CHG-0023: it documents document/image code grammar only and defers folder names to this file); `change-log/CHANGELOG.md` carries a new row above SHULL-CHG-0023's. **Not verifiable against live Drive from here** — that is out of scope for this record (see Risk, above), except that Physics Unit 01 is already known to match, because Matt built it there by hand. Status should be moved to IMPLEMENTED, and this field completed, only after the commit exists and has been diffed against this description.|
